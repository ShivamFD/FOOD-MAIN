...
// 🎥 Product Animation (Slide-in & Fade-in)
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (response.data && Array.isArray(response.data.products)) {
          setProducts(response.data.products);
          setMenuName(response.data.name || 'Menu Products');
        } else {
          setError('No products found for this menu.');
        }
      } catch (error) {
            ...
            {products.length > 0 ? (
              products.map((product, index) => (
                <ProductCard
                  ...
                  md={4}
                  lg={3}
                  style={{ animationDelay: `${index * 0.1}s` }} // Staggered animation effect
                >
                  <div style={{ position: 'relative' }}>
                    {product.discount && <OfferTag>{`₹${product.discount} OFF`}</OfferTag>}
                    <ProductImage
                      src={`http://localhost:5000${product.image}`}
                      alt={product.name}
                      onClick={() => handleProductClick(product._id)}
                  ...
                  <ProductDetails>
                    ...
                    <PriceTag>₹{product.price.toFixed(2)} for one</PriceTag>
                    <DeliveryTime>{product.deliveryTime || `${Math.floor(Math.random() * (59 - 20 + 1)) + 20} min`}</DeliveryTime>
                  </ProductDetails>
                </ProductCard>
              ))
