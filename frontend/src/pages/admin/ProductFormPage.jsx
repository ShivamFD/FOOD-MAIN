...
const ProductFormPage = () => {
  ...
  const isUpdateMode = !!productId; // Check if we're in update mode
  ...
  const submitHandler = async (e) => {
    ...
    // Ensure productId is present for update
    ...
    const productData = {
      ...
      price: Number(price),
      countInStock,
      menuId: menuId || null, // If no menu is selected, send null
    };

    console.log("Product Data being sent to API:", productData); // Debugging
    ...
    try {
      ...
      if (isUpdateMode) {
        ...
        const { data } = await createProduct(productData);
        console.log("Create Response:", data); // Debugging
        toast.success(data.message);
      }
      navigate("/admin/product-list"); // Redirect after success
    } catch (error) {
      console.error("Product Save Error:", error);
      toast.error(error?.data?.message || error.error);
    }
